...
if __name__ == "__main__":
    ...
    personLIST = [inputLIST[0], inputLIST[2], inputLIST[4], inputLIST[6]]
    dtarget =  inputLIST[10]
    coRefDICT = {dtarget:[]}

    for i in range(len(personLIST)):
        resultBOOL = coRefResolver(inputLIST, dtarget, personLIST[i])
        if resultBOOL == True:
            coRefDICT[dtarget].append(personLIST[i])
